...
def print_to_file(filename, unique_list_finished):
    ...
    f.close()
    print(
        "[+] Menyimpan wordlist dengan nama \033[1;32m"
        + filename
        + "\033[1;m, terhitung ada \033[1;32m"
        + str(lines)
        + " kata.\033[1;m"
    )
    print(
        "[+] Siap, sekarang wordlist \033[1;32m"
        + filename
        + "\033[1;m sudah siap digunakan! Semoga Beruntung! (filenya ada di internal storage)"
    )


def print_cow():
    print(" ___________ ")
    print(" \033[07m  cupicupita! \033[27m             # Cuppy")
    print("      \                     # Wordlist")
    print("       \   \033[1;32m,__,\033[1;m             # Maker")
    print("        \  \033[1;32m(\033[1;moo\033[1;32m)____\033[1;m         # Indonesia")
    print("           \033[1;32m(__)    )\ \033[1;m  ")
    print(
        "           \033[1;32m   ||--|| \033[1;m\033[05m*\033[25m\033[1;m      [ Muris Kurgas & Mebus ]"
    )
    print(5 * " " + "[ Translate by ETHEREAL| source: https://github.com/Mebus/]\r\n")


def version():
    """Display version"""

    print("\r\n	\033[1;32m[ cupp.py ]  " + __version__ + "\033[1;m\r\n")
...
def improve_dictionary(file_to_open):
    ...
    print("\r\n      *************************************************")
    print("      *                    \033[1;32mWARNING!!!\033[1;m                 *")
    ...
    print("\r\n[+] Sedang memproses wordlist...")

    print("[+] Mengurutkan dan menghapus duplikat...")
...
def interactive():
    ...
    print("\r\n[+] Masukkan informasi terkait target untuk membuat Wordlist!")
    print("[+] Klo gk tau, tekan enter aja... ;)\r\n")

    # We need some information first!

    profile = {}

    name = input("> Nama depan: ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] Setidaknya isi nama aslinya...")
        name = input("> Name: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> Nama belakang: ").lower()
    profile["nick"] = input("> Nama Panggilan: ").lower()
    birthdate = input("> Ulang Tahun (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] Masukin 8 Diqit angka! Tanggal bulan tahun!")
        birthdate = input("> Ulang Tahun (DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> Nama pacar/teman dekat: ").lower()
    profile["wifen"] = input("> Panggilan pacar/teman: ").lower()
    wifeb = input("> Ultah pacar/teman (DDMMYYYY): ")
    ...
    profile["kid"] = input("> Nama Anak/Artis/Siapapun: ").lower()
    profile["kidn"] = input("> Panggilan Anak/Artis/Siapapun: ").lower()
    kidb = input("> Ultah Anak/Artis/Siapapun (DDMMYYYY): ")
    while len(kidb) != 0 and len(kidb) != 8:
        print("\r\n[-] Masukin 8 Diqit angka! Tanggal bulan tahun!")
        kidb = input("> Ultah Anak/Artis/Siapapun: ")
    profile["kidb"] = str(kidb)
    print("\r\n")

    profile["pet"] = input("> Nama hewan peliharaan: ").lower()
    profile["company"] = input("> Nama tempat kerja/fanbase: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> Mau nambahin beberapa kata tentang target? Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
            "> Masukkan kata, dipisah dengan koma [cth. tampan,cantik,ganteng], spasi bakal dihapus: "
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "> Mau nambahin karakter spesial (&,@,#) dibelakang password? Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "> Mau nambahin angka2 di belakang password? Y/[N]:"
    ).lower()
    profile["leetmode"] = input("> Mau coba kombinasi alay? (cth. j0n4th4n) Y/[N]: ").lower()
...
def generate_wordlist_from_profile(profile):
    ...
    print("[+] Mengurutkan kata dan menghapus duplikat...")
...
def main():
    ...
    read_config(os.path.join(os.path.dirname(os.path.realpath(__file__)), "cupita.cfg"))
